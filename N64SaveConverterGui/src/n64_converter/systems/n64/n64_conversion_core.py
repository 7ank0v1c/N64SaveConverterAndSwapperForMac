# systems/n64/n64_conversion_core.py
...
def convert_save(path, src, src_type, tgt, tgt_type, byteswap_option, trim_pad_option, log_box=None):
    ...
    if not path or not os.path.exists(path):
        if log_box:
            log("Invalid input path.", log_box, level="ERROR")
        messagebox.showerror("Error", "Please select a valid input file.")
        return None

    key = f"{src}-{src_type}-{tgt}-{tgt_type}"
    if log_box:
        log(f"Starting conversion: {path}", log_box, key, level="INFO")

    data = read_bytes(path)
    if not data:
        log("Error reading input file.", log_box, key, level="ERROR")
        return None
    ...
    if conv:
        ...
        if log_box:
            log("No matching conversion found; using raw copy.", log_box, key, level="WARN")
    ...
    if tgt == NATIVE_LABEL:
        ...
        extension = os.path.splitext(path)[1]
        if log_box:
            log("Target is Native — using direct copy.", log_box, key, level="INFO")
